 ...
 *    analyse() passes i18n.language to streamResumeReview(), which forwards it
 *    to POST /api/resume/analyze-stream as the `language` field. The backend
...
function UploadView({ file, setFile, jobRole, setJobRole, jobAd, setJobAd, marketMode, setMarketMode, reviewContext, setReviewContext, onAnalyse, onSample }) {
  ...
  // reviewContext.js's option labelKeys ('context.unknown', 'context.stage.
  // student', ...) are useLanguage()'s dictionary keys, not useTranslation()'s
  // (i18next) — Preparation.jsx already reads them through useLanguage() for
  // its candidate-stage field, and translations.js has no 'context.*' entries
  // at all, so resolving them with the i18next `t` above renders the raw key
  // ("context.unknown") instead of the label. tCtx is only for o.labelKey.
  ...
  const coverageItems = [
    ...
    [t('resumeReview.covers.languageGrammar.title'), t('resumeReview.covers.languageGrammar.desc')],
  ...
  return (
    <>
      <section className="page-head rr-head">
        <div className="shell">
          <h1 className="page-head__title">{t('resumeReview.title')}</h1>
          <p className="page-head__sub">{t('resumeReview.subtitle')}</p>
        </div>
      </section>
      ...
      <div className="page-body rr-body">
        <div className="rr-main">
          ...
          <div className="market-mode-card">
            <div className="market-mode-title">{t('resumeReview.marketMode.title')}</div>
          ...
          <div className="context-card">
            <div className="context-card__title">{t('resumeReview.context.title')}</div>
            ...
            <div className="context-card__grid">
              {contextFields.map(([key, label, options]) => (
                <div className="form-group" key={key}>
                  ...
                  >
                    {options.map(o => (
                      <option key={o.value} value={o.value}>{tCtx(o.labelKey)}</option>
                    ))}
                  </select>
                </div>
              ))}
...
export default function ResumeReview() {
  ...
  async function analyse(fileArg, languageOverride) {
    ...
    await streamResumeReview(f, {
      ...
      // user just answered, rather than i18n.language: setLang()'s effect
      // that calls i18next.changeLanguage() runs on the next render, so
      // reading i18n.language here — in the same tick as the dialog's
      // onSelect — could still see the previous language. Falling back to
      // i18n.language covers handleReanalyse(), which never shows the dialog
      // when re-running with the language already established.
      language: languageOverride ?? i18n.language,
      onPartial: (partial) => {
        setFeedback(partial)
        setView('results')
      },
