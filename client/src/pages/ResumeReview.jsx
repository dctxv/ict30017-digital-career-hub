 ...
 * 2. Localisation — all static UI copy (labels, buttons, hints) goes through
 *    useLanguage()'s t(). The sample review ("View sample") is static demo
 *    copy written into this file, not real AI output, so it IS translated —
 *    SAMPLE_EN / SAMPLE_BN below, chosen from the selected language at render
 *    so flipping the toggle while viewing it swaps the text.
 *    Real AI-generated feedback (an actual analysed resume) is translated too:
 *    analyse() passes the selected language to streamResumeReview(), which
 *    forwards it to POST /api/resume/analyze-stream as `language`. The backend
 *    (ai-service/src/prompt/language.js) instructs the model to write the
 *    narrative fields in Bangla while keeping scores, enum values and the
 *    language_grammar original/corrected quote pairs in English — see that
 *    file for why. This was "on hold" until this change wired the frontend
 *    call site to actually send the language it already had.
 */
...
import LanguagePromptDialog from '../components/LanguagePromptDialog'
 ...
 * selected language and re-picks it if the user switches language
...
function describeQuota(quota, isAuthenticated, t, n) {
  ...
  // Two keys rather than a plural rule: English needs "review" vs "reviews" and
  // the dictionary has no plural machinery, while Bangla draws no such
  // distinction and uses the same sentence for both. n() renders the figures in
  // the selected script, so a Bengali sentence does not carry ASCII digits.
...
function UploadView({ file, setFile, jobRole, setJobRole, jobAd, setJobAd, marketMode, setMarketMode, reviewContext, setReviewContext, onAnalyse, onSample }) {
  ...
  const contextFields = [
    ['applicationChannel', t('review.contextChannel'), APPLICATION_CHANNEL_OPTIONS],
    ['employerType', t('review.contextEmployer'), EMPLOYER_TYPE_OPTIONS],
    ['candidateStage', t('review.contextStage'), CANDIDATE_STAGE_OPTIONS],
    ['targetSector', t('review.contextSector'), TARGET_SECTOR_OPTIONS],
  ]

  const coverageItems = [
    [t('review.coversContent'), t('review.coversContentDesc')],
    [t('review.coversLanguage'), t('review.coversLanguageDesc')],
    [t('review.coversFormat'), t('review.coversFormatDesc')],
    [t('review.coversAts'), t('review.coversAtsDesc')],
  ]

  return (
    <>
      <section className="page-head rr-head">
        <div className="shell">
          <h1 className="page-head__title">{t('nav.resumeReview')}</h1>
          <p className="page-head__sub">{t('review.sub')}</p>
        </div>
      </section>

      <div className="page-body rr-body">
        <div className="rr-main">
          ...
          {/* Visible whether or not a file has been chosen yet, rather than
              appearing only after upload — filling these in first and then
              dropping the file last works just as well, and gating them
              behind the file hid the whole form from anyone still deciding
              what to upload. */}
          <div className="market-mode-card">
            <div className="market-mode-title">{t('review.marketTitle')}</div>
            <div className="market-mode-options">
              ...
              >
                <span className="market-mode-btn__radio" />
                <span className="market-mode-btn__text">
                  <span className="market-mode-btn-label">{t('review.marketLocal')}</span>
                  <span className="market-mode-btn-desc">
                    {t('review.marketLocalDesc')}
                  </span>
                </span>
              </button>
              <button
              ...
              >
                ...
                <span className="market-mode-btn__text">
                  ...
                  <span className="market-mode-btn-desc">
                    {t('review.marketInternationalDesc')}
                  </span>
                </span>
              </button>
            </div>
          ...
          <div className="context-card">
            <div className="context-card__title">{t('review.contextTitle')}</div>
            <p className="context-card__hint">
              {t('review.contextHint')}
            </p>
          ...
          <div className="upload-actions">
            <button className="btn btn-primary btn-full" onClick={onAnalyse} disabled={!file}>
              {t('review.analyse')}
            </button>
        ...
        <aside className="rr-sidebar">
          <div className="val-card">
            ...
            <ul className="val-list">
              {coverageItems.map(([title, desc]) => (
                <li key={title} className="val-item">
                  <CheckCircle2 size={18} className="val-item__icon" />
                  <div className="val-item__text">
                    <div className="val-item__title">{title}</div>
                    <div className="val-item__desc">{desc}</div>
                  </div>
                </li>
              ))}
            </ul>
          ...
          <div className="sample-card">
            ...
            <button type="button" className="sample-card__link" onClick={onSample}>
...
export default function ResumeReview() {
  ...
  // Holds the file an Analyse/Re-analyse click wants to submit while the
  // language prompt below is open. `undefined` means the dialog is closed;
  // `null` means "use whatever's in the `file` state" (a plain re-analyse).
  const [pendingAnalyseFile, setPendingAnalyseFile] = useState(undefined)

  async function analyse(fileArg, languageOverride) {
    ...
    await streamResumeReview(f, {
      ...
      marketMode,
      reviewContext,
      // The backend already writes the narrative feedback in this language
      // (ai-service/src/prompt/language.js) — it just needs to be told which
      // one. Scores, enums and the language_grammar quote/correction pairs
      // stay in English regardless; that split is enforced in the prompt, not
      // here.
      //
      // languageOverride comes straight from the language-prompt dialog the
      // user just answered, rather than `lang`: setLang() schedules a state
      // update, so reading `lang` here — in the same tick as the dialog's
      // onSelect — would still see the previous language. Falling back to
      // `lang` covers handleReanalyse(), which never shows the dialog when
      // re-running with the language already established.
      language: languageOverride ?? lang,
      onPartial: (partial) => {
        setFeedback(partial)
        setView('results')
      },
      ...
      onError: (code, msg) => {
        ...
        setFeedback(current => {
          if (current) {
            setStreamError(msg || t('review.streamStoppedEarly'))
            setView('results')
          } else {
  ...
  // Gate the first Analyse click behind an explicit language choice, rather
  // than trusting that the navbar toggle was already set correctly — the
  // mix-up this exists to prevent (an analysis coming back in English right
  // after the user meant to switch to বাংলা) came from that toggle being
  // easy to forget, not from anything wrong in the translation plumbing.
  function requestAnalyse(fileArg) {
    setPendingAnalyseFile(fileArg instanceof File ? fileArg : null)
  }

  function confirmAnalyseLanguage(chosenLang) {
    const fileArg = pendingAnalyseFile
    setPendingAnalyseFile(undefined)
    setLang(chosenLang)
    analyse(fileArg, chosenLang)
  }

  function cancelAnalyseLanguage() {
    setPendingAnalyseFile(undefined)
  }
  ...
  /*
   * Which sample to show is decided at render rather than stored.
   *
   * It was previously written into `feedback` in two places — once when the
   * sample opened, and again from an effect watching the language, so that
   * flipping the EN/BN toggle mid-view did not leave stale-language text on
   * screen. That second write is a setState in an effect body, which React
   * flags as a cascading render, and it kept two sources of truth for one
   * value. The sample is static in both languages, so it is a pure function of
   * the language and belongs in the render path. Real analysed feedback still
   * comes from `feedback` and is untouched.
   */
  const displayedFeedback = isSample ? (lang === 'bn' ? SAMPLE_BN : SAMPLE_EN) : feedback
  ...
  return (
    <div className="rr-page">
      ...
      {pendingAnalyseFile !== undefined && (
        <LanguagePromptDialog
          titleKey="langPrompt.resumeTitle"
          subKey="langPrompt.resumeSub"
          onSelect={confirmAnalyseLanguage}
          onCancel={cancelAnalyseLanguage}
        />
      )}
    </div>
  )
