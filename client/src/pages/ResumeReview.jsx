      ...
      <section className="rr-upload">
        <div className="rr-upload__main">
          ...
          <div className="rr-submit">
            {/* Disabled once the day's allowance is spent, rather than letting
                the click travel to the server to be refused with a 429. The
                label beside it already says why; a guest's allowance is
                counted per address on the server, so only a signed-in count
                is trusted here. */}
            <button
              type="button"
              className="btn btn--primary btn--lg"
              onClick={() => onAnalyse()}
              disabled={!file || (quota?.authenticated && quota.remaining === 0)}
            >
              {t('review.analyse')}
              <Sparkles size={17} />
            </button>
...
export default function ResumeReview() {
  ...
  // The file an Analyse click wants to submit while the language prompt is
  // open. `undefined` means the dialog is closed; `null` means "use whatever
  // is in the `file` state".
  ...
  async function analyse(fileArg, languageOverride) {
    ...
    await streamResumeReview(target, {
      ...
      //
      // languageOverride is the answer the user just gave in the language
      // prompt. setLang() schedules a state update, so reading `lang` in the
      // same tick would still see the previous language. Re-analyse skips the
      // prompt and falls back to `lang`, which by then is settled.
  ...
  /*
   * The first Analyse click asks which language the review should be written
   * in, rather than trusting that the navbar toggle was already set the way the
   * user meant. The review is generated once and never retranslated, so this
   * is the one moment the choice still matters — and the mix-up it prevents
   * (an English review right after meaning to switch to বাংলা) came from that
   * toggle being easy to forget, not from the translation plumbing.
   */
  ...
  /*
   * The sample is chosen at render rather than stored in `feedback`. It is
   * static in both languages, so it is a pure function of the language toggle,
   * and reading it here is what keeps the EN/BN switch working while the sample
   * is on screen. Real analysed feedback still comes from `feedback`.
   */
  const displayedFeedback = isSample ? sampleReview(lang) : feedback
    ...
    <div className="page-enter rr">
      ...
      {view === 'results' && (
        <ResultsView
          ...
          onUploadNew={handleUploadNew}
          onNewFile={(next) => { setFile(next); requestAnalyse(next) }}
