...
export default function Preparation() {
  ...
  const openPast = async (row) => {
    ...
    try {
      ...
      setAnswers(Object.fromEntries(stored.map(entry => [entry.index, entry.answer])))
      ...
      setLive({
        ...
        position: Math.max(
          questions.findIndex(item => !(
            stored.find(entry => Number(entry.index) === Number(item.index))?.answer ?? ''
          ).trim()),
          0,
        ),
